Partial Class GF_spmtNewSBH
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/SPMT_Main/App_Display/DF_spmtNewSBH.aspx"
  ...
  End Sub
  Protected Sub GVspmtNewSBH_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVspmtNewSBH.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim IRNo As Int32 = GVspmtNewSBH.DataKeys(e.CommandArgument).Values("IRNo")  
        Dim RedirectUrl As String = TBLspmtNewSBH.EditUrl & "?IRNo=" & IRNo
        Response.Redirect(RedirectUrl)
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVspmtNewSBH_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVspmtNewSBH.Init
    DataClassName = "GspmtNewSBH"
    SetGridView = GVspmtNewSBH
  End Sub
  ...
  Protected Sub F_PurchaseType_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles F_PurchaseType.TextChanged
    ...
    InitGridPage()
  End Sub
  Protected Sub F_IsgecGSTIN_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles F_IsgecGSTIN.SelectedIndexChanged
    Session("F_IsgecGSTIN") = F_IsgecGSTIN.SelectedValue
    InitGridPage()
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    If Not Session("F_TranTypeID") Is Nothing Then
      ...
      End If
    End If
    F_IsgecGSTIN.SelectedValue = String.Empty
    If Not Session("F_IsgecGSTIN") Is Nothing Then
      If Session("F_IsgecGSTIN") <> String.Empty Then
        F_IsgecGSTIN.SelectedValue = Session("F_IsgecGSTIN")
      End If
    End If
    ...
    Dim validateScriptBPID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_SPMT_newSBH_BPID_main = true;" & _
      "    validate_FK_SPMT_newSBH_BPID(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_SPMT_newSBH_BPID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_SPMT_newSBH_BPID(o) {" & _
      "    var value = o.id;" & _
      "    var BPID = $get('" & F_BPID.ClientID & "');" & _
      "    try{" & _
      "    if(BPID.value==''){" & _
      "      if(validated_FK_SPMT_newSBH_BPID.main){" & _
      ...
      "    PageMethods.validate_FK_SPMT_newSBH_BPID(value, validated_FK_SPMT_newSBH_BPID);" & _
      "  }" & _
      "  validated_FK_SPMT_newSBH_BPID_main = false;" & _
      "  function validated_FK_SPMT_newSBH_BPID(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_SPMT_newSBH_BPID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_SPMT_newSBH_BPID", validateScriptFK_SPMT_newSBH_BPID)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
