...
Partial Class GF_spmtNewPAApproval
  ...
  Partial Class gvBase
    Inherits SIS.SYS.GridBase
  End Class
  Private WithEvents gvspmtPaymentAdviceCC As New gvBase
  Private Sub GVspmtPaymentAdvice_Init(sender As Object, e As EventArgs) Handles GVspmtPaymentAdvice.Init
    gvspmtPaymentAdviceCC.DataClassName = "GspmtPaymentAdvice"
    gvspmtPaymentAdviceCC.SetGridView = GVspmtPaymentAdvice
  End Sub

  Private Sub TBLspmtPaymentAdvice_Init(sender As Object, e As EventArgs) Handles TBLspmtPaymentAdvice.Init
    gvspmtPaymentAdviceCC.SetToolBar = TBLspmtPaymentAdvice
  End Sub
  ...
  Protected Sub GVspmtNewPA_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVspmtNewPA.RowCommand
    If e.CommandName.ToLower = "Approvewf".ToLower Then
      Try
        ...
        Dim Remarks As String = CType(GVspmtNewPA.Rows(e.CommandArgument).FindControl("F_ApprovalRemarks"), TextBox).Text
        SIS.SPMT.spmtNewPA.ApproveWF(AdviceNo, Remarks)
    ...
    If e.CommandName.ToLower = "Returnwf".ToLower Then
      Try
        Dim AdviceNo As Int32 = GVspmtNewPA.DataKeys(e.CommandArgument).Values("AdviceNo")
        Dim Remarks As String = CType(GVspmtNewPA.Rows(e.CommandArgument).FindControl("F_ApprovalRemarks"), TextBox).Text
        SIS.SPMT.spmtNewPA.ReturnWF(AdviceNo, Remarks)
        GVspmtNewPA.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message)
        Dim script As String = String.Format("alert({0});", message)
  ...
  Protected Sub GVspmtPaymentAdvice_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVspmtPaymentAdvice.RowCommand
    If e.CommandName.ToLower = "Approvewf".ToLower Then
      Try
        ...
        Dim Remarks As String = CType(GVspmtPaymentAdvice.Rows(e.CommandArgument).FindControl("F_ApprovalRemarks"), TextBox).Text
        SIS.SPMT.spmtPaymentAdvice.ApproveWF(AdviceNo, Remarks)
    ...
    If e.CommandName.ToLower = "Returnwf".ToLower Then
      Try
        Dim AdviceNo As Int32 = GVspmtPaymentAdvice.DataKeys(e.CommandArgument).Values("AdviceNo")
        Dim Remarks As String = CType(GVspmtPaymentAdvice.Rows(e.CommandArgument).FindControl("F_ApprovalRemarks"), TextBox).Text
        SIS.SPMT.spmtPaymentAdvice.ReturnWF(AdviceNo, Remarks)
        GVspmtPaymentAdvice.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message)
        Dim script As String = String.Format("alert({0});", message)
...
#Region " Common to Both Grid "
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    If Session("F_Pending") IsNot Nothing Then F_Pending.Checked = Convert.ToBoolean(Session("F_Pending"))
