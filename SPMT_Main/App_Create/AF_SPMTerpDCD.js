...
var script_SPMTerpDCD = {
    ...
    validate_ChallanID: function(sender) {
      ...
      this.validated_FK_SPMT_erpDCD_ChallanID_main = true;
      this.validate_FK_SPMT_erpDCD_ChallanID(sender,Prefix);
      },
    validate_HSNSACCode: function(sender) {
      var Prefix = sender.id.replace('HSNSACCode','');
      this.validated_FK_SPMT_erpDCD_HSNSACCode_main = true;
      this.validate_FK_SPMT_erpDCD_HSNSACCode(sender,Prefix);
      },
    validate_FK_SPMT_erpDCD_ChallanID: function(o,Prefix) {
      var value = o.id;
      var ChallanID = $get(Prefix + 'ChallanID');
      if(ChallanID.value==''){
        if(this.validated_FK_SPMT_erpDCD_ChallanID_main){
      ...
      value = value + ',' + ChallanID.value ;
        ...
        PageMethods.validate_FK_SPMT_erpDCD_ChallanID(value, this.validated_FK_SPMT_erpDCD_ChallanID);
      },
    validated_FK_SPMT_erpDCD_ChallanID_main: false,
    validated_FK_SPMT_erpDCD_ChallanID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_SPMTerpDCD.validated_FK_SPMT_erpDCD_ChallanID_main){
    ...
    validate_FK_SPMT_erpDCD_HSNSACCode: function(o,Prefix) {
      var value = o.id;
      var BillTypeID = $get(Prefix + 'BillTypeID_DDLspmtBillTypes');
      if(BillTypeID.value==''){
        if(this.validated_FK_SPMT_erpDCD_HSNSACCode_main){
      ...
      value = value + ',' + HSNSACCode.value ;
        ...
        o.style.backgroundRepeat= 'no-repeat';
        o.style.backgroundPosition = 'right';
        PageMethods.validate_FK_SPMT_erpDCD_HSNSACCode(value, this.validated_FK_SPMT_erpDCD_HSNSACCode);
      },
    validated_FK_SPMT_erpDCD_HSNSACCode_main: false,
    validated_FK_SPMT_erpDCD_HSNSACCode: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_SPMTerpDCD.validated_FK_SPMT_erpDCD_HSNSACCode_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
